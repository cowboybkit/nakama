...
func main() {
	...
	case "redis":
		...
		sessionCache = server.NewSessionCacheRedis(config, logger, db, tracker)
	default:
	...
	matchRegistry := server.NewLocalMatchRegistry(logger, startupLogger, config, sessionRegistry, tracker, router, metrics, config.GetName(), db)
